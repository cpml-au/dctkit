...
def coboundary_closure(c):
    # FIXME: works only for 2D case. Add docs
    # compute d_1^T ones
    dim = c.complex.dim
    num_tets = c.complex.S[dim].shape[0]
    num_dual_faces = c.complex.S[dim-1].shape[0]

    ones = CochainD0(complex=c.complex, coeffs=jnp.ones(num_tets))
