...
def circumcenter(
    S: npt.NDArray, node_coords: npt.NDArray
    ...
    A = np.zeros(
        (num_p, num_nodes_per_spx + 1, num_nodes_per_spx + 1), dtype=float_dtype
    )
    ...
    # NOTE: reshaping b so that it is treated as a batch of column vectors is needed
    # with numpy 2.0.
    bary_coords_extended = np.linalg.solve(A, b[:, :, None])
    bary_coords_extended = bary_coords_extended.reshape((num_p, num_nodes_per_spx + 1))
