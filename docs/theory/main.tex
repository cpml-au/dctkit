...
\paragraph{The coboundary closure} The \textbf{coboundary closure} is a critical modification to the dual 1-coboundary operator necessary for maintaining physical and mathematical consistency at domain boundaries. It was first introduced in \cite{mohamed2016discrete}. While the standard dual 1-coboundary $d^* _1$ is designed to evaluate the circulation dual 1-cochains along the boundaries of dual 2-cells (polygons), it encounters a limitation for nodes located on the physical boundary of a mesh. For these boundary nodes, the dual 2-cells are "open" because their boundaries are partially composed of primal edges. To resolve this, a supplementary boundary operator, denoted as $d_b$, is introduced to "close" these dual cells by traversing along the primal boundary edges in the direction of the dual cell's orientation. The coboundary closure ensures that discrete circulation and conservation properties are preserved across the entire computational domain.
...
\paragraph{Coboundary of a Vector-Valued Cochain} A natural way to define the coboundary of a vector-valued cochain is ``component-wise'', in the sense that if $\mathbf{c}^p = \sum_i \cc^p(\sigma^i_p)  \sigma^i_p$ is a vector-valued $p$-cochain and $d$ is the usual coboundary operator for scalar-valued cochain, then
\begin{equation}
    d \cc^p := \sum_i \cc^p(\sigma^i_p) d \sigma^p_i. \label{cobvec}
\end{equation}
Geometrically, we are adding up the contributions of basic $p$-cochains to obtain a $(p+1)$-cochain. On the contrary, we also want to preserve the relationship between continuous and discrete world, in the sense that vector-valued cochains should be the discrete counterpart of vector-valued differential forms. 

Berwick-Evans et al \cite{berwick2021discrete} defined the discrete exterior covariant derivative and in the special case of trivial bundles this coincides with \eqref{cobvec}. Their definition of a discrete $d_\nabla$ is
...
thus $d_\nabla$ is the adjoint with respect to chain-cochain pairing of the boundary operator. Since also the coboundary operator defined in \eqref{cobvec} satisfies this property, they must coincide.

\subsection{Hirani Flat Operators} 
We report here some of the discrete flats operators defined in \cite{hirani2003discrete} for vector-valued cochains. In the following, $\sigma_k \succ \sigma_l$ is the set of all the $k$-simplices in which the $l$-simplex $\sigma_l$ belongs to (for more details, see \cite{hirani2003discrete}).

\begin{Def}
    Let $c^0$ be a dual $0$-cochain. We define the \textbf{DPD flat} $\flat_{\text{DPD}}: \mathcal{C}^0(\star K, \rr) \rightarrow \mathcal{C}^1(\star K, \rr)$ as
    \begin{equation}
        \flat_{\text{DPD}}(c^0)(\star \sigma_{n-1}) := \sum_{\sigma_n \succ \sigma_{n-1}} c(\star \sigma_n) \cdot (\star \sigma_{n-1} \cap \sigma_n),
    \end{equation}
    where $(\star \sigma_{n-1} \cap \sigma_n)$ is the portion of the vector $\star \sigma_{n-1}$ in $\sigma_n$.
\end{Def}

\begin{Def}
    Let $c^0$ be a dual $0$-cochain. We define the \textbf{DPP flat} $\flat_{\text{DPP}}: \mathcal{C}^0(\star K, \rr) \rightarrow \mathcal{C}^1( K, \rr)$ as
    \begin{equation}
        \flat_{\text{DPP}}(c^0)(\sigma_{1}) := \sum_{\sigma_n \succ \sigma_1} \frac{|\star \sigma_1 \cap \sigma_n|}{|\star \sigma_1|} c^0(\star \sigma_n) \cdot \Vec{\sigma}_1,
    \end{equation}
    where $\Vec{\sigma}_1$ is the vector corresponding to $\sigma_1$ with the inherited direction and orientation.
\end{Def}

\begin{Def}
    Let $c^0$ be a primal $0$-cochain. We define the \textbf{PDP flat} $\flat_{\text{PDP}}: \mathcal{C}^0(K, \rr) \rightarrow \mathcal{C}^1( K, \rr)$ as
    \begin{equation}
         \flat_{\text{PDP}}(c^0)(\sigma_{1}) := \frac{c(\sigma^0_0) + c(\sigma^1_0)}{2} \cdot \Vec{\sigma}_1,
    \end{equation}
    where $\sigma^0_0$ and $\sigma^1_0$ are the nodes belonging to $\sigma_1$.
\end{Def}
