...
def test_elastica(is_bilevel=False):
    ...
    density = 1
    num_fem_elements = num_fem_elements_data*density
    S = get_elastica_mesh(num_fem_elements, 1)
    ...
    if is_bilevel:
        theta_0 = 0.1*np.random.rand(num_fem_elements-1).astype(dt.float_dtype)
        ...
        def energy_elastica_constr(theta: np.array, B: np.array) -> float:
            ...
            const = C.CochainD0(complex=S, coeffs=A *
                                np.ones(num_fem_elements, dtype=dt.float_dtype))
...
def test_elastica_data(data_type, is_bilevel=False):
    ...
    if is_bilevel:
        theta_0 = 0.1*np.random.rand(num_nodes-1).astype(dt.float_dtype)
        ...
        '''
        def obj_fun(theta_guess: np.array, E_guess: np.array) -> float:
            theta_guess = jnp.insert(theta_guess, 0, 0)
            # recover x and y
            cos_theta_guess = h_norm*jnp.cos(theta_guess)[:-1]
            sin_theta_guess = h_norm*jnp.sin(theta_guess)[:-1]
        ...
        prb = optctrl.OptimalControlProblem(
            objfun=obj_fun, state_en=energy_elastica_constr, state_dim=num_nodes-1)
    ...
    cos_theta = h_norm*jnp.cos(theta)[:-1]
    sin_theta = h_norm*jnp.sin(theta)[:-1]
